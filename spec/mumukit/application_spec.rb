...
describe Mumukit::Platform::Application do
  it { expect(Mumukit::Platform.laboratory.organic_url_for 'foo', '/foo/baz').to eq 'http://localmumuki.io/foo/foo/baz' }

  it { expect(Mumukit::Platform.classroom_ui.domain).to eq 'classroom.localmumuki.io' }
  it { expect(Mumukit::Platform.classroom_ui.organic_url_for 'org', '/foo/baz').to eq 'http://classroom.localmumuki.io/#/org/foo/baz' }
  ...
  it { expect(Mumukit::Platform.application.organic_url_for 'org', '/bar').to eq 'http://sample.app.com/org/bar' }

  describe Mumukit::Platform::Application::Basic do
    context 'with path mapping strategy' do
      let(:mapping) { Mumukit::Platform::PathMapping }
  ...
  describe Mumukit::Platform::Application::Organic do
    context 'with subdomain mapping strategy' do
      let(:mapping) { Mumukit::Platform::PathMapping }
      it { expect(Mumukit::Platform::Application::Organic.new('http://foo.com:3000').organic_url('bar')).to eq 'http://foo.com:3000/bar/' }
    end

    context 'with path mapping strategy' do
      let(:mapping) { Mumukit::Platform::PathMapping }
      it { expect(Mumukit::Platform::Application::Organic.new('http://foo.com:3000').organic_url('org')).to eq 'http://foo.com:3000/org/' }
      it { expect(Mumukit::Platform::Application::Organic.new('http://foo.com:3000/foo').organic_url('org')).to eq 'http://foo.com:3000/foo/org/' }
      it { expect(Mumukit::Platform::Application::Organic.new('http://foo.com:3000/app/').organic_url_for('an_org', 'some/long/path')).to eq 'http://foo.com:3000/app/an_org/some/long/path' }
      it { expect(Mumukit::Platform::Application::Organic.new('http://foo.com:3000/another_app/').organic_url_for('another_org', 'some/long/path?with=value')).to eq 'http://foo.com:3000/another_app/another_org/some/long/path?with=value' }

      context 'with fragments' do
        context 'with prefragment-path' do
          let(:fragmented) { Mumukit::Platform::Application::Organic.new('http://foo.com:3000/classroom/#/') }
        ...
        context 'without fragment-path' do
          let(:fragmented) { Mumukit::Platform::Application::Organic.new('http://foo.com:3000/#') }
